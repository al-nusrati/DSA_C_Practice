...
private:
    ...
    // --- Search Node (Optimized for Reusability) ---
...
public:
    ...
    // Reusability Fix: Return bool instead of printing inside the class
    bool search(int val) { return searchNode(root, val); }

    // Reusability Fix: Return bool
...
int main() {
    ...
    if (bst.search(6)) cout << "Node 6 FOUND." << endl;
    else cout << "Node 6 NOT found." << endl;

    if (bst.search(99)) cout << "Node 99 FOUND." << endl;
    else cout << "Node 99 NOT found." << endl;
    ...
    // Notice how we pass <int> to tell the template what type we expect!
    int userVal = getValidInput<int>("Enter an integer to insert into BST: ");
    bst.insert(userVal);
    cout << "Inorder after user insert: "; bst.inorder(); cout << endl;
